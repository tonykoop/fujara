...
- Keep the packet at **L2 V5 build-packet candidate**.
- Focus on long-boreside geometry and side-branch coupling paths that affect effective pipe length.
...
- Side-branch air path relation to the labium remains the key variable that should be measured before any authoritative CAD/CADX updates.
- Main-bore station geometry and end-condition behavior are tracked as measurement blockers until `validation-loop` rows are populated.

## Authority Rules for this Packet

- `family-spec.csv` is the packet authority register and currently declares a single hypothesis family: **open bore + side branch excitation**.
...
1. Confirm acoustic-law row and side-branch hypothesis in `family-spec.csv`.
